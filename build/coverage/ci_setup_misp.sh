...
# setting is enabled" - and the refusal is not an error exit, so the loop above
# silently left it at its default of true. Setting it in the loop therefore did
# nothing, and Event::publishRouter went on enqueueing to a queue that this job
# starts no worker to drain.
